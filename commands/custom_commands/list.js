...
module.exports = class ListCommand extends Command {
	...
	run(msg) {
		const allCommandsFromServer = sql
			.prepare(
				`SELECT * FROM commands WHERE server_id = '${
					msg.guild.id
		...
		if (allCommandsFromServer.length == 0) {
			return sendErrorResponse(
				msg,
				"There are no commands added for this server! Please add some using the `$add` command. See `$help `for more information."
			);
		}

		var commandCounter = 1;
		for (var command in allCommandsFromServer) {
			...
			embed.addField(
				`$${commandName} ${acceptsArgs === "true" ? "[args]" : ""}`,
				`**ID:** ${commandId}`,
				inline
			);
