...
module.exports = class UnknownCommandCommand extends Command {
	...
	run(msg, { cmd }) {
		...
		const incrementUseCounter = sql.prepare(
		...
		if (getCommandFromDb.get(cmd, argsFlag, msg.guild.id)) {
			...
			if (
				argsFlag === "true" &&
				getCommandFromDb.get(cmd, "true", msg.guild.id)
			) {
				var response = getCommandFromDb.get(cmd, argsFlag, msg.guild.id)
					.response;
				if (isUrl(response)) {
					for (var i = 1; i < argsArray.length; i++) {
				...
				return msg.say(
					getCommandFromDb.get(cmd, argsFlag, msg.guild.id).response
				);
			}
		}
		function isUrl(s) {
